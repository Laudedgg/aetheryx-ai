    ...
    <div className="max-w-2xl mx-auto py-4 space-y-3">
      ...
      <div style={cardStyle} className="p-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button onClick={() => { if (liveMode) toggleMode() }}
            ...
            style={{
              background: !liveMode ? 'rgba(138,108,255,0.06)' : 'rgba(255,255,255,0.015)',
              border: !liveMode ? '2px solid rgba(138,108,255,0.4)' : '2px solid rgba(255,255,255,0.06)',
              boxShadow: !liveMode ? '0 0 20px rgba(138,108,255,0.1)' : 'none',
            }}
          >
            {!liveMode && <span className="absolute top-3 right-3 w-2 h-2 rounded-full animate-pulse" style={{background:'#8A6CFF',boxShadow:'0 0 8px rgba(138,108,255,0.5)'}} />}
            <div className="w-9 h-9 rounded-xl flex items-center justify-center mb-3" style={{background:'rgba(138,108,255,0.1)'}}>
      ...
      <div style={cardStyle} className="p-4">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" style={{background: liveMode ? 'rgba(52,211,153,0.08)' : 'rgba(138,108,255,0.08)'}}>
            <span className="text-[14px]">{liveMode ? '✅' : '💡'}</span>
          </div>
          <div>
            <p className="text-[13px] font-semibold" style={{color: liveMode ? '#34d399' : '#8A6CFF'}}>{liveMode ? 'Live Mode Active' : 'Demo Mode Active'}</p>
