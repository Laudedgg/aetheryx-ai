      ...
      <div className="relative min-h-screen overflow-hidden">
        ...
        <nav className="relative z-10 border-b border-white/[0.06]">
          <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 py-3 sm:py-5 max-w-[90rem] mx-auto">
            ...
            <ul className="hidden xl:flex items-center gap-8">{["Features","Agents","Platform","Integrations","Pricing"].map(i=><li key={i}><a href={`#${i.toLowerCase()}`} className="text-sm text-[#b3b3b3] hover:text-white transition-colors">{i}</a></li>)}</ul>
            <div className="flex gap-2 items-center">
              <a href="#pricing" className="btn-ghost-blink hidden xl:inline-flex text-sm">See Pricing</a>
              <Link href={APP_URL} className="btn-blink hidden xl:inline-flex text-sm px-6 py-3">Get Started</Link>
              <button className="xl:hidden p-2 text-white/70 hover:text-white rounded-lg hover:bg-white/[0.06] transition-colors" onClick={()=>setMenuOpen(!menuOpen)}>
          ...
          {menuOpen && (
            <div className="xl:hidden border-t border-white/[0.06] bg-[#060a14]/98 backdrop-blur-xl px-5 py-3 pb-5">
